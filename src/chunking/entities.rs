...
impl ChunkEntities {
    ...
    pub fn entity(&self, pos: ChunkLocation) -> Option<Entity> {
    ...
    pub fn iter(&self) -> impl Iterator<Item = (&ChunkLocation, &Entity)> {
        self.0.iter()
    }

    /// Returns an iterator iterating over the loaded chunk keys.
    pub fn iter_keys(&self) -> impl Iterator<Item = &ChunkLocation> {
        self.0.keys()
    }
...
impl DirtyChunks {
    ...
    pub fn iter_dirty(&self) -> impl Iterator<Item = &ChunkLocation> {
        self.0.iter()
    }
